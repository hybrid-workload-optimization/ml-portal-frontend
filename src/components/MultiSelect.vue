<template>
  <div class="sp-multi-select">
    <div class="multi-select__box">
      ...
      <sp-select
        v-if="firstSelectMeta && secondSelectMeta && thirdSelectMeta"
        class="multi-select--select"
        background-color="#F5F5f5"
        hide-details="auto"
        placeholder="- Select"
...
export default {
  ...
  methods: {
    ...
    async initSecondItems() {
      if (!this.secondSelectMeta) {
        return
      }
      await this.requestGetSecondItems(this.secondSelectMeta)
      const secondItem = sessionStorage.getItem('secondVal')
      if (this.secondItems && this.secondItems.length > 0) {
        if (
          secondItem &&
          this.secondItems.some(e => Number(e.id) === Number(secondItem))
        ) {
          this.onChangeSecondSelect(secondItem, true)
        } else if (this.autoSelect) {
          // session storage의 id 값이 items에 없을 경우
          if (secondItem) {
            sessionStorage.removeItem('secondVal')
          }
          this.onChangeSecondSelect(this.secondItems[0].value, true)
        }
        this.initThirdItems()
    ...
    async initThirdItems() {
      ...
      if (this.thirdItems && this.thirdItems.length > 0) {
        if (
          thirdItem &&
          this.thirdItems.some(e => Number(e.id) === Number(thirdItem))
        ) {
          this.onChangeThirdSelect(thirdItem)
        } else if (this.autoSelect) {
          // session storage의 id 값이 items에 없을 경우
          if (thirdItem) {
            sessionStorage.removeItem('thirdVal')
          }
          this.onChangeThirdSelect('')
        }
      }
    },
    onClickBtn() {
    ...
    async onChangeSecondSelect(value, init = false) {
    ...
    async onChangeThirdSelect(value) {
      await this.setThirdValue(value)
      this.emitChageItem()
    },
    emitChageItem() {
