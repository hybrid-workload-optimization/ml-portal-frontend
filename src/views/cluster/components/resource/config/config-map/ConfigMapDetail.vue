...
export default {
  ...
  computed: {
    ...configMapUtils.mapGetters(['detailInfo']),
    titleData() {
      return {
  ...
  methods: {
    ...configMapUtils.mapActions(['getDetail']),
    ...configMapUtils.mapActions(['getYaml']),
    ...configMapUtils.mapActions(['updateConfigMap']),
    ...
    async onClickDelConfirm() {
      try {
        ...
        if (response.status === 200) {
          ...
          // 1초 후 리스트 화면으로 이동
          setTimeout(() => this.$router.push('/config/config-map/list'), 1000)
        } else {
          // 삭제 실패 시
