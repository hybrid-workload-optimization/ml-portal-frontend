...
export default {
  ...
  computed: {
    ...pvcMapUtils.mapGetters(['dataList']), // replicaSetList.js에 저장된 dataList 값 반환(replicaSetList.js)
    ...pvcMapUtils.mapGetters(['dataListSize']), // replicaSetList.js에 저장된 dataListSize 값 반환(replicaSetList.js)
  },
  ...
  methods: {
    ...
    openYamlEditor() {
      ...
      this.openModal({
        ...
        isEncoding: false,
        content: '',
        title: 'New Persistent Volume Claim',
    ...
    moveToDetailPage(data) {
      ...
      if (persistentVolumeClaimIdx) {
        this.$router.push(
          `/config/persistent-volume-claim/detail/${persistentVolumeClaimIdx}`,
        )
      }
    },
    ...
    async onChangeItem(value) {
      if (value.secondValue) {
        ...
        // 데이터 리스트 초기화
        this.initPersistentVolumeClaimDataList()
      }
