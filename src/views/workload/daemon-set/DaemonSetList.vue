...
export default {
  ...
  computed: {
    ...daemonSetMapUtils.mapGetters(['dataList']), // replicaSetList.js에 저장된 dataList 값 반환(replicaSetList.js)
    ...daemonSetMapUtils.mapGetters(['dataListSize']), // replicaSetList.js에 저장된 dataListSize 값 반환(replicaSetList.js)
  },
  ...
  methods: {
    ...
    ...daemonSetMapUtils.mapActions(['getList']), // 리스트 조회 요청(daemonSet.js)
    ...daemonSetMapUtils.mapActions(['createDaemonSet']), // DaemonSet 생성 요청(daemonSet.js)
    ...daemonSetMapUtils.mapMutations([
      'initDaemonSetState',
      'initDaemonSetDataList',
    ]), // state 데이터 초기화(replicaSetList.js)
    ...
    moveToDetailPage(data) {
      ...
      if (daemonSetIdx) {
        this.$router.push(`/workload/daemon-set/detail/${daemonSetIdx}`)
      }
    },
    ...
    async onConfirmedFromEditModal(value) {
      ...
      try {
        ...
        const response = await this.createDaemonSet(param)
    ...
    async onChangeItem(value) {
      if (value.secondValue) {
        ...
        // TODO 추후 통일성을 위해 수정 필요
        await this.getListData(value)
      } else {
        // 데이터 리스트 초기화
        this.initDaemonSetDataList()
      }
